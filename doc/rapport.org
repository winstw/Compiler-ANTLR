...
À cette structure, nous avons ajouté les éléments nécessaires à l'interprétation, par exemple, le corps des méthodes et leurs paramètres, et les valeurs que prennent les variables durant l'évaluation du programme.
...
Ces différentes étapes sont lancées à partir de la méthode compile() de Main.java dans la mesure où l'étape précédente ne
...
Par exemple lors d'une assignation, elle va vérifier que le type de l'expression à gauche de ~:=~ est bien le même que celui de
...
paliers des détails des différentes étapes du projet (consignes par
